...
class answera extends R{
	...
	*/
	...
	private function exec(){
		...
		//检查进行中的试题数组Session
		if(!isset($_SESSION['list_anonymous']))$this->json(2);
		...
		//获得试题量
		$count = count($list);
		//取得第一题-------------如果对数组Session list_anonymous结构不清楚，请参阅/yepo/model/underwaya.class.php的注释
		for($i = 0; $i < $count; $i++){
		...
		//检测是否为考试，是：记录考试log
		if(!$db->iQ("insert into `anonylog`(`id`,`ip`,`qid`,`time`)values(NULL,'".$this->ip()."',$next,'".time()."')"))$this->json(6);

		//记录已回答
		for($i = 0; $i < $count; $i++){
			//记录该题已回答，跳出循环
			if($list[$i][2] == 0){
				$_SESSION['list_anonymous'][$i][2] = $res;
				break;
			}
		}
